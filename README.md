...
# livecoding-kubernetes
### Construir y probar las imágenes Docker que utilizaremos:
...
docker container ls
Escribe en tu navegador localhost:8080
Borra el contenedor:
Docker rm -f basica_00


docker build -t hola-kubernetes/basica:1.1 .
docker run -p 8080:80 --name basico_11 -d hola-kubernetes/basica:1.1

docker build -t hola-kubernetes/blue:blue .
docker run -p 8080:80 --name basico_blue -d hola-kubernetes/blue:blue

docker build -t hola-kubernetes/green:green .
docker run -p 8080:80 --name basico_green-d hola-kubernetes/green:green

Borrar los contenedores (no las imágenes!):
docker rm - f 
